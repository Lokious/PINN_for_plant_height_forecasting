                ...
                for rescale in ['True']:
                    for weight in [0]:
                        ...
                        with open(job_file, 'w') as file1:
                            ...
                            file1.writelines("#SBATCH --time=0-01:30:00\n")
...
def run_sbatch_cpu(genetics_embedding=False):
        ...
        for mode in ['pinn_lstm_smooth_temp_new_final']:
            for if_pinn in [True]:
                if genetics_embedding:
                    genotype_list = [[33, 106, 122, 133, 5, 30, 218, 2, 17, 254, 282, 294, 301, 302, 335, 339, 341, 6, 362]]
                    ...
                    genotype_list = [33, 106, 122, 133, 5, 30, 218, 2, 17, 254, 282, 294, 301, 302, 335, 339, 341, 6, 362]#301, 302, 335, 339, 341, 6, 362]#122, 133, 5, 30, 218, 2, 17, 254, 282, 294
...
def run_sbatch_pinn_cross_validation(smooth_temp=False):
    ...
    for genotype in genotype_list:
        ...
        with open(job_file, 'w') as file1:
            ...
            file1.writelines("#SBATCH --time=0-24:00:00\n")
            file1.writelines("#SBATCH --cpus-per-task=1\n")
            ...
            file1.writelines('conda run -n test\n')
            file1.writelines(
                "python3 Single_genotype_NN_PINN.py -if_pinn True -genotype {0} -smooth_temp {1}\n"
                .format( genotype,smooth_temp))
            file1.close()
        # os.system("sbatch %s" % job_file)
        os.system("python3 Single_genotype_NN_PINN.py -if_pinn True -genotype {0} -smooth_temp {1} &\n"
                .format( genotype,smooth_temp))
        os.system('rm {}'.format(job_file))
...
def run_multiple_genotype_model(mode_name,if_pinn=True,smooth_loss=False,reduce_time_resolution=False,smooth_input=False
    ...
    job_file=mode_name
    if if_pinn ==True:
        for split_group in ['g_e']:#,,'year_site.harvest_year','genotype.id'
            ...
            for genotype_encoding in encoding_list:#
                for smooth_input in [smooth_input]:
                    ...
                    with open(job_file, 'w') as file1:
                        ...
                        file1.writelines(
                            "python3 Multiple_genotype_model_NN_PINN.py -mode {0} -if_pinn {1} -smooth_loss {2} -genotype_encoding {3} -split_group {4} -reduce_time_resolution {5} -smooth_input {6} -smooth_temp {7}\n"
                            .format(job_file, if_pinn,smooth_loss,genotype_encoding,split_group,reduce_time_resolution,smooth_input,smooth_temp))
                    file1.close()
                    print("python3 Multiple_genotype_model_NN_PINN.py -mode {0} -if_pinn {1} -smooth_loss {2} -genotype_encoding {3} -split_group {4} -reduce_time_resolution {5} -smooth_input {6} -smooth_temp {7}\n"
                            .format(job_file, if_pinn,smooth_loss,genotype_encoding,split_group,reduce_time_resolution,smooth_input,smooth_temp))
                    # os.system("sbatch %s" % job_file)
                    os.system("python3 Multiple_genotype_model_NN_PINN.py -mode {0} -if_pinn {1} -smooth_loss {2} -genotype_encoding {3} -split_group {4} -reduce_time_resolution {5} -smooth_input {6} -smooth_temp {7}\n"
                            .format(job_file, if_pinn,smooth_loss,genotype_encoding,split_group,reduce_time_resolution,smooth_input,smooth_temp))
                    # time.sleep(7200)
    else:
        for split_group in [ 'year_site.harvest_year', 'genotype.id','g_e']: #'year_site.harvest_year', 'genotype.id','g_e'
            if split_group == 'year_site.harvest_year':
...
def main():
    # run_sbatch_pinn_cross_validation(smooth_temp=True)
    run_multiple_genotype_model(mode_name='pinn_result_NEW',if_pinn=True,smooth_loss=False,
                                reduce_time_resolution=False,smooth_temp=True)
    ...
    # run_multiple_genotype_model(mode_name='NN_result_NEW',if_pinn=False,smooth_loss=False,reduce_time_resolution=False,
    #                             smooth_temp=True)
