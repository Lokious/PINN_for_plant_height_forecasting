...
def run_logistic_ode_pinn(data_path='test_seq.csv', save_directory='/pinn_result/figure/', mode: str = '',
    ...
    try:
        #dataframe to same result
        predicte_value_curve = pd.read_csv('pinn_result/predict_curve_{}.csv'.format(mode), header=0,index_col=0)
        print('connect result to existing predited curve .csv file')
    except:
        print('pinn_result/predict_curve_{}.csv Do not exist, create new file'.format(mode))
        predicte_value_curve = pd.DataFrame()
    predicte_value_gradient = pd.DataFrame()
    predicte_value_r = pd.DataFrame()

    for n in [0]:#range(n_split):# different split
                            ...
                            for ode_int_loss in ode_int_loss_list:
                                for L2 in [1.0]: #1.0,0.1
                                    ...
                                    for j in randomseed_list:  # different random seed
                                        ...
                                        if 'log' in mode:
                                            predict_y_train_detach = reverse_min_max_scaling(
                                                copy.deepcopy(predict_y_train.detach()), data_min=scaler[0],
                                                data_max=scaler[1])
                                            predict_y_train_detach = reverse_log_tensor(predict_y_train_detach)

                                            predict_y_validation_detach = reverse_min_max_scaling(
                                                copy.deepcopy(predict_y_validation.detach()),
                                                data_min=scaler[0],
                                                data_max=scaler[1])
                                            predict_y_validation_detach = reverse_log_tensor(predict_y_validation_detach)
                                            predict_y_test_detach = reverse_min_max_scaling(copy.deepcopy(predict_y_test.detach()),
                                                                                            data_min=scaler[0],
                                                                                            data_max=scaler[1])
                                            predict_y_test_detach = reverse_log_tensor(predict_y_test_detach)

                                            copy_y_train = reverse_min_max_scaling(copy_y_train, data_min=scaler[0],
                                                                                   data_max=scaler[1])
                                            copy_y_train = reverse_log_tensor(copy_y_train)
                                            copy_y_test = reverse_min_max_scaling(copy_y_test, data_min=scaler[0],
                                                                                  data_max=scaler[1])
                                            copy_y_test = reverse_log_tensor(copy_y_test)
                                            copy_y_validation = reverse_min_max_scaling(copy_y_validation, data_min=scaler[0],
                                                                                        data_max=scaler[1])
                                            copy_y_validation = reverse_log_tensor(copy_y_validation)
                                        else:
                                        ...
                                        try:
                                            # dataframe to same result
                                            predicte_value_curve = pd.read_csv(
                                                'pinn_result/predict_curve_{}.csv'.format(mode), header=0,
                                                index_col=0)
                                            print('connect result to existing predited curve.csv file')
                                        except:
                                            print('no file exist')
                                        for seq_n in range(predict_y_test_detach.shape[1]):
                                            ...
                                            predicte_value_curve = pd.concat([predicte_value_curve, new_predict_curve], axis=1)
                                            predicte_value_curve.to_csv('pinn_result/predict_curve_{}.csv'.format(mode))

                                            # col_name = '{}split_hidden{}_env{}_ts{}_lr_{}_w_ph{}_ode_int_{}_l2_{}_{}_rs_{}_seq{}'.format(
                                            #     n, hidden, num_layer, num_layer, lr, weight_physic,ode_int_loss, L2,mode, j, seq_n)
                                            # new_predict_gradient = pd.DataFrame(
                                            #     data={col_name: torch.squeeze(y_gradient_temperature.detach()[:, seq_n, 0]).cpu()},
                                            #     index=range(y_gradient_temperature.detach().shape[0]))
                                            # predicte_value_gradient = pd.concat([predicte_value_gradient, new_predict_gradient],
                                            #                                     axis=1)
                                            if multiple_r:
                                                new_predict_r = pd.DataFrame(
                                                    data={col_name: torch.squeeze(
                                                        r_value_pred.detach()).cpu()},
                                                    index=range(predict_y_train.detach().shape[0]))
...
def fit_ode_for_seq_seperatelly_plot(data_path,genotype, start_day,years,corrected=False,result_df=None ):
    ...
    result_df.to_csv('logistic_genotype_average_rmse.csv')
    return result_df
...
class fit_logistic_ode_to_plant_height():
    ...
    def logistic_growth_ode(self,y, time,theta):
        ...
        dY_dT = r * y * (1 - (y / y_max))
        return dY_dT
    def fit_ode(self,time:torch.tensor,fit_data,color,ax):
        """
        ...
        self.plot_data_and_fit_curve(fit_data, time,fitted_curve,color=color,ax=ax)
    ...
    def fit_dataset_and_save(self,data:torch.tensor,seq_label:pd.DataFrame,start_day=56):
        ...
        plt.ylim(-0.1, 1.5)
        plt.xlim(0, 285)
        for seq,label in zip(range(num_seq),seq_label['genotype.id']):
            print(seq)
            ...
            # print(ts_seq.shape)
            c= color_list[seq]
            result_df, fitted_curve = self.fit_ode(ts_seq,seq_data,color=c,ax=ax)
        ...
        # fit_ode_result = fit_ode_result[['r', 'y_max']].reset_index()
        # fig, ax = plt.subplots(figsize=(12, 10))

        # plot1 = sns.jointplot(x='r',y='y_max', data=fit_ode_result)
        # plot1.ax_joint.cla()
        # # print(fit_ode_result[['r','y_max']])
        # for i in range(len(fit_ode_result.index)):
        #     print(fit_ode_result.loc[i,:])
        #     plot1.ax_joint.plot(fit_ode_result.loc[i,'r'], fit_ode_result.loc[i,'y_max'], color=color_list[i],marker='o')
        # plt.ylim(0, 1)
        # plt.xlim(0, 0.2)
        # plt.xlabel('r')
        # plt.ylabel('y_max')
        # plt.show()#r and y max distribution
        print(fit_ode_result)
        return fit_ode_result['mask_rmse'].mean()
    def plot_data_and_fit_curve(self,data,ts,fitted_curve,color='g',ax=None):
        data = copy.deepcopy(data)
        data[data==0.0]=np.nan
        sns.scatterplot(data,ax=ax,color=color)
...
def predict_based_on_ode(fitted_parameters, logistic_growth_ode, train_y, ts):
    ...
    return precit_y
def mask_spearman(true_y:torch.tensor, predict_y:torch.tensor):
    device = true_y.device
    mask = (~torch.isin(true_y, torch.tensor(0.0).to(device))).float()
    masked_true_y = mask * true_y
    masked_predict_y = mask * predict_y
    #drop missing values,
    print(masked_true_y.shape)
    print(masked_predict_y.shape)
    spear_corr=[]
    for seq in range(masked_true_y.shape[1]):
        seq1= copy.deepcopy(masked_true_y.detach())[:,seq]
        seq1 = seq1[seq1 != 0.0]
        seq2= copy.deepcopy(masked_predict_y.detach())[:,seq]
        seq2 = seq2[seq2 != 0.0]
        # print(seq1.shape)
        spear = SpearmanCorrCoef(num_outputs=1)
        spear_corr.append(spear(seq1, seq2))
        # print(spear_corr)
    # masked_true_y = masked_true_y[masked_true_y != 0.0]
    # masked_predict_y = masked_predict_y[masked_predict_y != 0.0]


    return sum(spear_corr)/len(spear_corr)
...
def ODE_fit_run():
    ...
    def plot_ODE_fit(g, precit_y_test, rmse_loss_test, shape_dtw_loss_test, test_y_avg, ts_test,add_name=''):
        ...
        plt.ylim(0, 1.5)
        plt.title('genotype {} RMSE:{} shape dtw:{}'.format(g, round(rmse_loss_test, 3), round(shape_dtw_loss_test, 3)))
        plt.savefig('../figure/logistic_ODE_fit/genotype{}{}logistic ODE test curves.png'.format(g,add_name))
        plt.clf()
    ...
    for g in [33, 106, 122, 133, 5, 30, 218, 2, 17, 254, 282, 294, 301, 302, 335, 339, 341, 6, 362]:
        ...
        all_ode_result_before_average.to_csv('logistic_ode_fit_multiple_genotype_raw.csv')
...
def plot_ode_fit_result(file_name):
    # plot
    from test import order_genotype_based_on_their_similarity
...
def load_best_hyperparameters_and_cross_validate(if_pinn=True, best_hyperparameter_file='best_model_result_summary/pinn_penalize_r_best_hyperparameters_result.csv',
    ...
    train_test_validation_dictionary = manually_data_split_based_on_one_group(group_df,split_group='year_site.harvest_year')
    n_split = len(train_test_validation_dictionary.keys())
    # assert n_split==6
    # raise EOFError
    for n in range(n_split):# different split range(n_split)
        ...
        for j in randomseed_list:  # different random seed
            ...
            run = wandb.init(
                # Set the project where this run will be logged
                project="PINN_single_genotype_{}".format(mode),
                # id='{0}'.format(mode),
                # Track hyperparameters and run metadata
                config={
                    "learning_rate": lr,
            ...
            with open(
                    'pinn_result/result_summary/best_model_cv/model/genotype_{}_trained_model_{}split_hidden{}_env{}_ts{}_lr_{}_w_ph{}_ode_int_{}_l2_{}_{}_rs_{}.dill'.format(
                        genotype,n, hidden, num_layer, num_layer, lr, weight_physic, ode_int_loss,
                        L2, mode, j), 'wb') as file:
                dill.dump(model, file)
            file.close()
...
def main():
    ...
    #     print(genotype)
    #     # penalize_r
    #     file1 = glob.glob("pinn_result/result_summary/single_genotype/PINN/PINN_mask_loss_pinn_lstm_corr_same_lengthgenotype{}penalize_rpinnmode_Truefit_ml_first_FalsecorrectedFalsesmoothFalsefill_in_naTruerescaleFalsestart_date_115.csv".format(genotype))
    #     print(file1)
    #     load_model_recalculate_prediction_rmse(file_name=file1[0],genotype=genotype)
    # #     #without penalize r
    # #     file2 = glob.glob("pinn_result/result_summary/single_genotype/PINN_mask_loss_pinn_aver_same_lengthgenotype{}pinnmode_Truefit_ml_first_FalsecorrectedFalsesmoothFalsefill_in_naTruerescaleFalsestart_date_*.csv".format(genotype))
    # #     print(file2)
    # #     load_model_recalculate_prediction_rmse(file_name=file2[0],genotype=genotype)
    #     file3 = glob.glob("pinn_result/result_summary/single_genotype/pure_ml/PINN_mask_loss_gpu_lstm_corr_same_lengthgenotype{}pinnmode_Falsefit_ml_first_FalsecorrectedFalsesmoothFalsefill_in_naTruerescaleFalsestart_date_115w0_.csv".format(genotype))
    #     print(file3)
    #     load_model_recalculate_prediction_rmse(file_name=file3[0],genotype=genotype)
    # plot_ode_fit_result('pinn_result/multipl_g/pinn_weight_2_multiple_g_parameters_fix_error_cpu_orthogonal_init_train_lr_0.001_hidden_5_num_layers_3_l2_0.5_seed_1.csv')

    # plot_ode_fit_result(file_name='best_model_result_summary/temperature_ode_fit_result_mean.csv')
    # with open('snps_embedding_model/cpu_best_same_lengthmultiple_genotypepinnmode_Falsefit_ml_first_FalsecorrectedFalsesmoothFalsefill_in_na_at_startTruerescaleFalsestart_date_91w5_genetics_input_5_2_7_5.dill','rb') as file:
