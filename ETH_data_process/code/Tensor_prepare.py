...
import random
import Read_data_from_csvfiles
...
def manually_data_split_based_on_one_group(group_df,split_group = 'year_site.harvest_year',n_splits=6):
    """
    ...
    if split_group=='year_site.harvest_year':
        ...
        for rand_seed in range(len(train_year_combination)):
            ...
            return train_test_validation_dictionary
    else:
...
def manually_split_on_two_groups(group_df):
    ...
    """
    train_test_validate_dictionary = manually_data_split_based_on_one_group(group_df,split_group='year_site.harvest_year')
    ...
    for split in train_test_validate_dictionary.keys():
        ...
        val_index = train_group[train_group['genotype.id'].isin(val_g)].index.to_list()
        test_index = train_group[train_group['genotype.id'].isin(test_g)].index.to_list()
...
class create_tensor_dataset():
    ...
    def read_yield_convert_to_tensor(self, genotype,
        ...
        yield_df = Read_data_from_csvfiles.data_load(yield_file, year=year)
...
class TestData_split(unittest.TestCase):
    def test_train_test_split_based_on_group(self):
        df_test = pd.DataFrame({"genotype.id":[1,1,2,3,4,2,3,4,5,6,3,3],
                                "year_site.harvest_year":[2019,2019,2018,2018,2017,2017,2019,2019,2022,2022,2021,2021]},
                               index=range(12))
        train_test_split_based_on_group(df=df_test,group_column=df_test['genotype.id'],group_name=['genotype.id'],n_split=5)
def main():
    unittest.main()
