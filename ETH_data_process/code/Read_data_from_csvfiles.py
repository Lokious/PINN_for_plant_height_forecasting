'''
This script is loads pre-process data, fit ode models with data
'''
...
def data_load(file_name,year:int|list = 2019):
    ...
    #print(len(single_year_experiment['timestamp'].unique()))
    # print(single_year_experiment.index)
    # print(single_year_experiment.duplicated(['timestamp','genotype.id','plot.UID','lot']).sum())
...
def load_multiple_year_data(height_file="C:/data_from_paper/ETH/olivia_WUR/process_data/Canopy coverage.csv",
    ...
    # #the difference in start measuring date for planr height
    # start_days_difference = [item1 - item2 for (item1, item2) in zip(start_date_list_canopy_coverage, start_date_list_plant_height)]
    # print(start_days_difference)
    #
    # #use canopy coverage measure as standard, align 'day_after_start_measure' for plant height
    # group_year = height_df_multiple_year.groupby('year_site.harvest_year')
    # aligned_height_df_multiple_year = pd.DataFrame()
    # for group,start_days_difference_single_year in zip(group_year.groups,start_days_difference):
    #     group_df = group_year.get_group(group)
    #     # set start dates as 0, add column 'day_after_start_measure'
    #     group_df.loc[:, 'day_after_start_measure'] = group_df.loc[:,'day_after_start_measure'] - start_days_difference_single_year.days
    #     aligned_height_df_multiple_year = pd.concat([aligned_height_df_multiple_year, group_df])
    # else:
    #     print(aligned_height_df_multiple_year)
    #
...
def solve_logistic_ode(ode_model:callable, data:pd.DataFrame, theta):
    ...
    for i, item in enumerate(groups_object.groups):
        ...
        #     return odeint(func=logistic_ode_model, y0=theta[-1:], t=input_data.timestamp, args=(theta,))
        #
        # theta = results.x  # least squares solution used to inform the priors
        #
        # # r_mu, r_std = norm.fit(theta[0])
        # # y_max_mu, y_max_std = norm.fit(theta[1])
        # # yt0_mu, yt0_std = norm.fit(theta[2])
        # posterior_predict_check(input_data, pytensor_forward_model_matrix, theta)
