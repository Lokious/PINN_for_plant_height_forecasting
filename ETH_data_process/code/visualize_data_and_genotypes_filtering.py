'''
This script is to combine traits (in different files) from multiple years experiment together, merge environment factors
author:Yingjie shao
Usage: python3 visualize_data_and_genotypes_filtering.py
...
def plot_height_coverage_together(height_file,coverage_file):
    ...
    # coverage_df['timestamp'] = pd.to_datetime(coverage_df['timestamp']).dt.date
    # drop rows which corrected value is NA
    # height_df.dropna(subset=['corrected_value'], inplace=True)
    # coverage_df.dropna(subset=['corrected_value'], inplace=True)
...
def find_genotype_present_at_multiple_years(df_name="../processed_data/align_height_env.csv",number_of_years=4)->list:
    """
    return genotype list, which present in all for years, should have length 19
    :param df_name:time series which has been aligned based on sowing date
    :param number_of_years:int, default 4
    :return:
    """
...
def genotype_present_in_specific_years(df_path="../processed_data/align_height_env.csv",year_list:list=[2018,2019,2021,2022])->list:
    ...
    for genotype in df['genotype.id'].unique():
        ...
        return gen_list
def main():
    # combine_multiple_year_experiments_save_files_for_plant_height_and_canopy_coverge()#read seperatefile combine to plant height and conapy coverage
    import pandas as pd
    ...
    # pdf = pd.read_csv('loss_ratio.csv',header=0,index_col=0)
    # pdf['d/p'] = (pdf['data_loss']/pdf['physic_loss'])
    # plt.plot(pdf['d/p'])
    # plt.xlabel('epochs')
    # plt.ylabel('ratio')
    # plt.title('Loss ratio ($L_d$/ $L_p$)')
    # plt.show()
    # plot_height_coverage_together("C:/data_from_paper/ETH/olivia_WUR/process_data/Plant height.csv","C:/data_from_paper/ETH/olivia_WUR/process_data/Plant height.csv")
    # read_file_environment_factors()
    # find_genotype_present_at_multiple_years()
    # df = pd.read_csv("C:/data_from_paper/ETH/olivia_WUR/process_data/Plant height.csv",header=0,index_col=0)
    # plot_multiple_environment_factors_lineplot(df)

    # combine_multiple_year_experiments_save_files_for_plant_height_and_canopy_coverge(directory="C:/data_from_paper/ETH/trait_repository_clone/")
    # plot_height_coverage_together("C:/data_from_paper/ETH/olivia_WUR/process_data/Canopy coverage.csv", "C:/data_from_paper/ETH/olivia_WUR/process_data/Plant height.csv")
    #read_file_environment_factors()
