...
from Tensor_prepare import reverse_min_max_scaling,train_test_split_based_on_group,train_test_split_based_on_two_groups, count_parameters,manually_data_split_based_on_one_group
import matplotlib.pyplot as plt
import seaborn as sns

from Tensor_prepare import minmax_scaler,manually_split_on_two_groups
from Single_genotype_NN_PINN import average_based_on_group_df,mask_rmse_loss,smooth_tensor_ignore_nan,plot_multiple_sequences_colored_based_on_label_df
...
def read_best_hyperparameter_retrain_with_different_data_split_seed(best_hyperparameter_file:str,
    ...
    if split_group == 'g_e':
        #split to predict new year, new genotype
        train_test_validation_dictionary = manually_split_on_two_groups(copy.deepcopy(group_df))
        model_name = model_name + split_group +'_split'
        # raise EOFError
    else:
        # split based on either year or genotype, if get two columns,
        # it can only make sure at least one of them is different for samples be splitted into different groups(train test etc.)
        train_test_validation_dictionary = manually_data_split_based_on_one_group(copy.deepcopy(group_df),split_group=split_group,n_splits=6)
        raise EOFError
        if split_group == 'year_site.harvest_year':
            model_name = model_name + 'year_split'
        elif split_group == 'genotype.id':
